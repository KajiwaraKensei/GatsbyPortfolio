...
const circle1 = (p: StyleProps) => keyframes`
  ...
  100%{
    stroke-dasharray: ${(p.circumference || CIRCUMFERENCE) * (p.percent * 0.01) + " " + (p.circumference || CIRCUMFERENCE)};
  }
  `;
...
const circle2 = (p: StyleProps) => keyframes`
  ...
  100%{
    stroke-dasharray:${p.circumference || CIRCUMFERENCE} ${p.circumference || CIRCUMFERENCE};
  }
`;
...
export default styled(Component) <StyleProps>`
  ...
  & .type2{
    stroke: ${p => p.marginalColor || (
    lighten((p.colorDifference || COLOR_DIFFERENCE), p.marginalColor || p.baseColor || BASE_COLOR)
  )};
